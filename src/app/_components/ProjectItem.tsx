...
export default function Projectitem(props: Project) {
  return (
    <article className="w-[95%] md:max-w-[850px] h-auto flex flex-col md:flex-row justify-center items-start md:justify-start md:items-center relative mr-4 bg-[#09090B] rounded-lg gap-x-7">
      ...
      <div className="w-full h-full flex flex-col items-start gap-2 text-start relative">
        ...
        <div className="w-full flex flex-row gap-x-2">
          ...
          {props.technologies.map((tech) => (
            <Chip variant="flat" className={`${tech.class}`} startContent={<tech.svg width={20} height={20}/>}>{tech.name}</Chip>
          ))}
        </div>
